...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode):
        self.mode = mode
        self.root_path = '/gs/fs/tga-i/share/data/image_datasets/RHD_published_v2'
        self.rootnet_output_path = '/gs/fs/tga-i/share/data/image_datasets/RHD_published_v2/rootnet_output'
        ...
        if self.mode == 'train':
            ...
            for i in range(41258): # number of images to use for training#41258
                if i in db.anns:
                    part[i] = db.anns[i]
        if self.mode == 'test':
            part = {} # part of dataset
            for i in range(2728): # number of images to use for training
                if i in db.anns:
                    part[i] = db.anns[i]
