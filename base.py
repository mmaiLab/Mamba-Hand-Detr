...
class Trainer(Base):
    ...
    def get_optimizer(self, model):
        ...
        # mamba_params = [p for name, p in model.named_parameters() if 'mamba' in name]
        # other_params = [p for name, p in model.named_parameters() if 'backbone' not in name and 'mamba' not in name]
        # optimizer = torch.optim.AdamW([
        #         {'params':backbone_params, 'weight_decay':0.0},
        #         {'params':mamba_params, 'weight_decay':0.00999},
        #         {'params':other_params}
        #     ], lr=cfg.lr, betas=(0.9, 0.999), weight_decay=1e-2)
        # optimizer = torch.optim.AdamW(model.parameters(), betas=(0.9, 0.999), weight_decay=0.01, lr=cfg.lr)
